...
// Generatorul wordmarkului Edulab58.
//
// COPIAT din `edumat58/scripts/edumat-wordmark-gen.mjs`, nu rescris: literele
// trebuie să fie ACELEAȘI forme, altfel cele două platforme nu mai arată a
// aceeași casă. Wordmarkul lor nu e text cu un font — sunt litere desenate,
// construite din inele, benzi de arc și dreptunghiuri, cu contrast gros/subțire
// dat de `OFF` (inelul interior decalat la dreapta).
//
// Ce am adăugat: `l` și `b`, care nu existau în „edumat58". Restul e neatins.
...
// l: doar stâlpul gros, de la ascender la baseline. Nu are bol, deci nici
// contrast — e litera cea mai simplă din alfabet și lățimea ei e chiar stâlpul.
// Un `l` lat cât celelalte ar fi lăsat o gaură în cuvânt.
function letterL() {
  return { w: STEM, d: rect(0, ASC, STEM, B - ASC) }
}

// b: oglinda lui `d` — același inel, stâlpul mutat la STÂNGA.
//
// Contrastul se oglindește și el: la `d` inelul interior e decalat cu +OFF, deci
// peretele gros cade în stânga, lângă bol. La `b` stâlpul e deja în stânga, așa
// că bolul trebuie să fie gros în DREAPTA — altfel litera are toată greutatea pe
// o parte și se răstoarnă vizual. De aceea inelul se desenează cu -OFF.
function letterB() {
  const cx = R
  const stem = rect(0, ASC, STEM, B - ASC)
  const icx = cx - OFF
  const ring =
    `M ${fmt(cx - R)} ${CY} A ${R} ${R} 0 1 1 ${fmt(cx + R)} ${CY} A ${R} ${R} 0 1 1 ${fmt(cx - R)} ${CY} Z ` +
    `M ${fmt(icx - RIN)} ${CY} A ${RIN} ${RIN} 0 1 0 ${fmt(icx + RIN)} ${CY} A ${RIN} ${RIN} 0 1 0 ${fmt(icx - RIN)} ${CY} Z`
  return { w: 2 * R, d: ring + ' ' + stem }
}
...
const word = [letterE(), letterD(), letterU(), letterL(), letterA({ star: true }), letterB(), digit5(), digit8()]
